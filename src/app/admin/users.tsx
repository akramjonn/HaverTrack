...
const SORTERS: Record<
  SortKey,
  (a: AdminRosterRow, b: AdminRosterRow) => number
> = {
  joined: (a, b) =>
...
export default function AdminUsersScreen() {
  ...
  const roster = useAdminRoster(debounced, offset);
  ...
  const onRefresh = async () => {
    ...
    await queryClient.invalidateQueries({
      queryKey: adminKeys.roster(debounced, offset),
    });
    setRefreshing(false);
  };
    ...
    >
      ...
      {roster.isLoading && !roster.data ? (
        ...
        <View style={styles.errorBox}>
          <Text style={styles.errorText}>
            {roster.error?.message || "Could not load students."}
          </Text>
        </View>
      ) : sorted.length === 0 ? (
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <View style={styles.metaRow}>
                    <Text style={styles.metaText}>
                      Joined {relativeTime(u.joined_at)}
                    </Text>
                    <Text style={styles.metaDot}>·</Text>
                    <Text style={styles.metaText}>
                      Active {relativeTime(u.last_active_at)}
                    </Text>
                  </View>
                </View>
      ...
      >
        ...
        <Text style={Typography.caption}>
          Page {offset / 50 + 1} · Sorting this page
        </Text>
        <Pressable
          disabled={(roster.data?.length ?? 0) < 50}
