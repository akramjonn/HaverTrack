...
Email/password signup requires an exact `@haverford.edu` address. A verified
campus email establishes email ownership, not enrollment;
distinguishing students from staff requires a college-provided roster or SSO claim.

## Hosted configuration

Google OAuth is disabled. Email confirmation is required
(`mailer_autoconfirm = false`). The native callback and Site URL are
`havertrack://auth/callback`.
...
On a device, test email registration, confirmation, resend and password login;
and cold-start callback handling. Open confirmation links on the device/browser used to register so
